...
public class RuConverter
{
    ...
    {
        return Thread.CurrentThread.CurrentCulture.TextInfo.ToTitleCase(_string);
    }
    /// <summary>
    ...
    /// <param name="_date"></param>
    /// <returns></returns>
    public string DateToTextLong(DateTime _date)
    {
        return string.Format("«{0}» {1} {2}",
                                _date.Day.ToString("D2"),
                                MonthName(_date.Month, TextCase.Genitive),
    ...
    /// <returns></returns>
    public string CurrencyToTxt(double _amount, bool _firstCapital)
    {
        //Десять тысяч рублей 67 копеек
